...
public interface IOrderService
{
    ...
    Task<OrderResponse> GetByIdAsync(int id, int actorUserId, IReadOnlyCollection<string> actorRoles, bool isStaff, CancellationToken cancellationToken = default);
    Task<OrderQuoteResponse> QuoteAsync(PlaceOrderRequest request, int actorUserId, CancellationToken cancellationToken = default);
    Task<OrderResponse> CreateAsync(PlaceOrderRequest request, int actorUserId, IReadOnlyCollection<string> actorRoles, CancellationToken cancellationToken = default);
