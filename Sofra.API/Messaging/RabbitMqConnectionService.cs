...
/// <summary>
/// Drzi jednu (singleton) RabbitMQ konekciju za zivot API procesa. Za razliku od Workerove verzije,
/// ako RabbitMQ ostane nedostupan i nakon svih pokusaja, API i dalje mora normalno da se pokrene i
/// posluzuje HTTP zahtjeve - publish jednostavno nece raditi dok se konekcija ne uspostavi.
/// </summary>
    ...
    ILogger<RabbitMqConnectionService> logger) : IHostedService, IAsyncDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 0; attempt <= BackoffSeconds.Length; attempt++)
        {
            ...
            {
                logger.LogError(ex,
                    "RabbitMQ nedostupan nakon {MaxAttempts} pokušaja - API nastavlja rad bez objavljivanja događaja dok se konekcija ne uspostavi.",
                    BackoffSeconds.Length);
                return;
            }
        }
    }
