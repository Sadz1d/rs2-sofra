    ...
    where TEvent : class, IEvent
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) => await OnMessageAsync(channel, deadLetterQueue, ea, stoppingToken);

        await channel.BasicConsumeAsync(queueName, autoAck: false, consumer, stoppingToken);

        logger.LogInformation("Consumer za {EventType} sluša red {Queue} (routing key {RoutingKey}).", typeof(TEvent).Name, queueName, routingKey);
